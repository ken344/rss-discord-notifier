...
- **FR-007-1**: RSSフィードに画像がある場合、サムネイル画像として表示する
- **FR-008**: 通知送信のレート制限に対応する
- **FR-009**: フィードごとに異なるDiscordチャンネル（Webhook URL）への通知を可能にする

### 2.3 状態管理機能
- **FR-010**: 最後に通知した記事の情報を永続化する
- **FR-011**: 初回実行時は最新N件のみを通知する（過去の全記事を通知しない）

### 2.4 設定管理機能
- **FR-012**: RSSフィードリストを設定ファイル（YAML/JSON）で管理する
- **FR-013**: Discord Webhook URLを環境変数で管理する
- **FR-014**: 通知頻度、取得件数などの設定をカスタマイズ可能にする
